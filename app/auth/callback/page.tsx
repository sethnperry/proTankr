...
export default function AuthCallbackPage() {
  ...
  useEffect(() => {
    // 1) If Supabase sent an error in the hash (common for expired/invalid links),
    // the server cannot see it — handle it here.
    ...
    // 2) If we have a PKCE code, do NOT exchange in the browser.
    // Forward to a server route that exchanges + sets cookies.
    ...
    // 3) Fallback: if a session already exists (rare), proceed.
    (async () => {
